...
### Fixed
- **Wrapper crash with AL extension 18.0.2668733 (prerelease)** -- that build moved `Microsoft.Dynamics.Nav.EditorServices.Host` and `almcp` out of the per-platform `bin/win32`/`bin/linux` subfolders into a flat `bin/`. The wrapper only looked in the old location, so it failed to spawn the AL Language Server, crashed on startup, and VS Code disabled it after 5 restarts. The wrapper now probes the flat layout first and falls back to the legacy one. On Windows that restores startup; see Known limitations below for Linux/macOS and for the .NET 10 requirement. The current stable AL extension (17.0.2273547) still uses the old layout and was never affected.

### Known limitations
- **AL extension 18.0.2668733 (prerelease) needs a machine-wide .NET 10 runtime on Windows, and is not yet supported on Linux/macOS.** That build is framework-dependent: it ships portable IL (`Host.dll`, `almcp.dll`) plus a Windows-only apphost, and no Linux or macOS native binaries at all. On Windows the apphost starts only when .NET 10 (both `Microsoft.NETCore.App` and `Microsoft.AspNetCore.App`) is installed machine-wide -- the runtime the official AL extension acquires for itself is private to VS Code and invisible to the wrapper -- and otherwise exits with "You must install .NET to run this application." On Linux and macOS there is no binary for the wrapper to launch at all. Launching via `dotnet <dll>`, which addresses both, is in progress. Older AL extensions, including the current stable 17.x, are unaffected.
